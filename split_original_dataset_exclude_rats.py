...
def return_train_and_test_set(DATASET, PATTERNS_EXCLUDED_TRAINING, SEED=7, test_pcnt=0.3):
    ...
    CLASSES = [k for k in set(file_list_all['label'])]
    N_CLASSES = len(CLASSES)
    ...
    def remove_rats_until_pcnt_met(ds, test_pcnt, SEED):
        ...
        ds1 = set(list(cages_and_groups_healthy.keys()))
        ds2 = set(list(cages_and_groups_pathological.keys()))
        ds_common = list(ds1.intersection(ds2))


        n_total = ds.shape[0]
        n_test_desired = round(n_total * test_pcnt)
        n_common = len(ds_common)

        n_excluded = 0
        excluded_rats = []
        random.seed(SEED)
        ds_train = ds
        ds_test = pd.DataFrame(columns=ds.keys())
        while n_excluded < n_test_desired:
            rat_id = random.randint(0, n_common-1)
            cage, group = ds_common[rat_id]
            if [cage, group] not in excluded_rats:
                ds_train = ds_train.query('cage != \'{}\' or group != \'{}\''.format(cage, group))
                ds_test = pd.concat([ds_test, ds.query('cage == \'{}\' and group == \'{}\''.format(cage, group))])
                excluded_rats.append([cage, group])
                n_excluded += cages_and_groups_healthy[cage][group] + cages_and_groups_pathological[cage][group]

        return ds_train, ds_test, len(ds_test)/n_total
    ...
    files_train, files_test, final_test_pcnt = remove_rats_until_pcnt_met(dataset_c, test_pcnt, SEED)
    ...
    print_distribution([f[0] + '/' + f[1] for f in files_train[['label', 'name']].get_values()], CLASSES)

    print("Test set distribution:")
    print_distribution([f[0] + '/' + f[1] for f in files_test[['label', 'name']].get_values()], CLASSES)
