...
testing_independent_dataset = False
# testing_independent_dataset = True
if testing_independent_dataset:
    TEST_FOLDER = 'input/D21'
    print('### TESTING INDEPENDENT DATASET! ###')
...
if args.json_parameters:
    ...
    # json_parameters = json.loads(base64.decode(args.json_parameters))
    experiment_name = json_parameters['experiment_name']
    DATASET = json_parameters['DATASET']
    PATTERNS_EXCLUDED_TRAINING = json_parameters['PATTERNS_EXCLUDED_TRAINING']
else:
    print('Error, no json params')
    exit()


files_train, files_test, final_pcnt_test = return_train_and_test_set(DATASET, PATTERNS_EXCLUDED_TRAINING, SEED)
...
if not (args.test or args.test_train):
    ...
    metric = "-val_acc{val_acc:.6f}-val_loss{val_loss:.6f}" + "-" + experiment_name
    monitor = 'val_acc'
    ...
    # TEST
    # if args.test:
    #     ids = glob.glob(join(TEST_FOLDER, '*.png'))
    ...
    csv_name = 'results_' + model_name + '_' + experiment_name + '.csv'

    # load test_labels
    # if not testing_independent_dataset:
    #     # with open(TEST_LABELS_FILE, 'r') as fp:
    #     #     reader = csv.reader(fp)
    #     #     test_labels = list(reader)
    #     test_labels = [inst[0] for inst in files_test]
    ...
    with conditional(args.test, open(csv_name, 'w')) as csvfile:
        ...
        if args.test:
            ...
            if testing_independent_dataset:
                csv_writer.writerow(['oct_name', 'predicted_class', 'prob_healthy', 'prob_pathological'])
            else:
                csv_writer.writerow(
                    ['oct_name', 'ground_truth', 'predicted_class', 'prob_healthy', 'prob_pathological', 'is_correct'])
            classes = []
        else:
            correct_predictions = 0

        for i, inst in enumerate(tqdm(ids)):
            ...
            img = cv2.resize(img, (CROP_SIZE, CROP_SIZE), interpolation=cv2.INTER_CUBIC)
            img = np.stack((img,) * 3, -1)
            img = img[np.newaxis, ...]  # np.expand_dims(img, axis=0)
            ...
            # check if prediction was good or not

            if testing_independent_dataset:
                test_predictions.append([idx, predicted_class, prediction[0][0], prediction[0][1]])
                csv_writer.writerow(test_predictions[-1])
            else:
                is_correct = 1 if label == predicted_class else 0
                test_predictions.append(
                    [idx, label, predicted_class, prediction[0][0], prediction[0][1], is_correct])
                csv_writer.writerow(test_predictions[-1])
                # for tl in test_labels:
                #     if tl[0].split('.')[0] == idx.split(os.sep)[-1:][0].split('.')[0]:
                #         is_correct = 1 if tl[1] == predicted_class else 0
                #         test_predictions.append(
                #             [tl[0], tl[1], predicted_class, prediction[0][0], prediction[0][1], is_correct])
                #         csv_writer.writerow(test_predictions[-1])

        # if args.test_train:
        #     print("Accuracy: " + str(correct_predictions / (len(transforms) * i)))

        if args.test and not testing_independent_dataset:
            ...
            print("\nRESULTS\n====================\n")
            ok = sum(1 for tc in test_predictions if tc[-1] == 1)
            nok = sum(1 for tc in test_predictions if tc[-1] == 0)
            print("Accuracy: {}".format(ok / (ok + nok)))
            K.clear_session()
